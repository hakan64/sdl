...
void Starfield::render(pp_uint8* buffer)
{
	...
	pp_int32 centerx = width*32768;
	pp_int32 centery = height*32768;
	...
	for (pp_int32 i = 0; i < numStars; i++)
	{
		...
		if (f.z > 0)
		{
			pp_int32 px = (((f.x<<10)/f.z)+centerx)>>16;/*(fpdiv(f.x*200, f.z) + centerx)>>16;*/
			pp_int32 py = (((f.y<<10)/f.z)+centery)>>16;/*(fpdiv(f.y*200, f.z) + centery)>>16;*/
...
void Starfield::update(pp_uint32 syncFrac)
{
	for (pp_int32 i = 0; i < numStars; i++)
	{
		stars[i].z-=/*syncFrac>>9;*/1;
		if (stars[i].z < 0)
		{
			stars[i].z += /*((65535>>2) + 2048);*/255;
		}
	}
}
